...
// 4th-order weights (embedded, for error estimate).
...
export function integrate(f, t0, tEnd, y0, opts) {
  opts = opts || {};
  ...
  while (t < tEnd) {
    ...
    if (h > hmax) h = hmax;
    if (h < hmin) {
      h = hmin; // floor the step to guarantee progress at extreme stiffness.
      if (bad) {
        // Accept a floored step to avoid stalling; recompute derivative.
        t += h;
